...
CSV.foreach("bse_bhav_copy.csv", :headers => true) do |row| 
	if row[3] == "Q" && ss_id > 1706 && ss_id < 2200
		t = Time.now
		stock = row
		...
		VALUES (?, ?)").execute(
			stock_split, 
			stock[1])
		file = open("http://www.bseindia.com/stockinfo/stockprc2_excel.aspx?scripcd=#{stock[0]}&FromDate=01/01/2009&ToDate=06/24/2014&OldDMY=D")
